...
Que hace:
  - Consulta periodicamente varias fuentes gratuitas:
      ...
      * Alpaca Market Data (gratis, cuenta paper) -> volumen y spread bid-ask del SPY,
        maximo/minimo semanal del dolar (UUP), y precio del oro (GLD) para la
        correlacion inversa dolar-oro
...
# Simbolo que usamos como proxy del oro (GLD = ETF que replica el precio del oro)
SIMBOLO_ORO = "GLD"
...
def _obtener_cambio_oro():
    """Devuelve el % de cambio del oro (proxy GLD) respecto al cierre del dia
    anterior, o None si algo falla. Se usa para la correlacion inversa con el dolar."""
    headers = {
        "APCA-API-KEY-ID": ALPACA_API_KEY,
        "APCA-API-SECRET-KEY": ALPACA_SECRET_KEY
    }
    url_snap = f"https://data.alpaca.markets/v2/stocks/{SIMBOLO_ORO}/snapshot"
    snap = requests.get(url_snap, headers=headers, timeout=10).json()
    try:
        precio_actual = snap["latestTrade"]["p"]
    ...
    try:
        ...
        if caida_pct >= UMBRAL_CAIDA_DOLAR and clave_caida not in _ya_notificado:
            ...
            if cambio_oro is None:
                ...
                linea_oro = f"El oro responde: subio {cambio_oro}% -- coherente con la relacion inversa dolar-oro."
            elif cambio_oro < 0:
                linea_oro = f"El oro responde: bajo {abs(cambio_oro)}% -- no sigue la relacion inversa habitual, revisalo."
            else:
...
# ---------------------------------------------------------------------------
# ORO: reporte periodico en USD y EUR (XAU/USD y XAU/EUR, via proxy GLD)
# ---------------------------------------------------------------------------

def _obtener_eur_por_dolar():
    """Frankfurter.app: tipo de cambio USD->EUR, gratis y sin necesidad de llave
    (datos del Banco Central Europeo). Devuelve None si falla."""
    try:
        ...
        print(f"Error Frankfurter EUR/USD: {e}")
        return None


def revisar_estado_oro():
    """Manda a Discord el estado actual del oro (proxy GLD) en dolares y en
    euros, con el cambio porcentual del dia en cada moneda. No espera a que
    se cumpla ningun umbral -- es un reporte periodico, no una alerta."""
    if "PON_AQUI" in ALPACA_API_KEY:
        return
    ...
    try:
        url_snap = f"https://data.alpaca.markets/v2/stocks/{SIMBOLO_ORO}/snapshot"
        snap = requests.get(url_snap, headers=headers, timeout=10).json()
        precio_usd = snap["latestTrade"]["p"]
        cierre_anterior_usd = snap["prevDailyBar"]["c"]
        cambio_pct = round((precio_usd - cierre_anterior_usd) / cierre_anterior_usd * 100, 2)

        eur_por_dolar = _obtener_eur_por_dolar()
        if eur_por_dolar is None:
            linea_eur = "No se pudo consultar el tipo de cambio EUR/USD en este momento."
        else:
            precio_eur = round(precio_usd * eur_por_dolar, 2)
            cierre_anterior_eur = round(cierre_anterior_usd * eur_por_dolar, 2)
            linea_eur = (
                f"Oro en euros (proxy): {precio_eur} EUR "
                f"(cierre anterior: {cierre_anterior_eur} EUR)"
            )

        flecha = "subio" if cambio_pct > 0 else ("bajo" if cambio_pct < 0 else "se mantiene igual")

        enviar_discord(
            "Estado del oro (USD / EUR)",
            f"Oro en dolares (proxy {SIMBOLO_ORO}): {precio_usd} USD "
            f"(cierre anterior: {cierre_anterior_usd} USD)\n"
            f"{linea_eur}\n\n"
            f"Hoy el oro {flecha} {abs(cambio_pct)}% en dolares.\n\n"
            f"Nota: se usa {SIMBOLO_ORO} como proxy del oro, no es el precio "
            f"literal de la onza troy (XAU), pero sigue su movimiento muy de cerca.",
            color=0xE8B84B
        )
    except Exception as e:
        print(f"Error Alpaca/Frankfurter estado oro: {e}")
...
def enviar_prueba_completa():
    if "PON_AQUI" in ALPACA_API_KEY:
        enviar_discord(
            "Prueba: faltan las llaves de Alpaca",
            "No puedo leer datos de mercado porque ALPACA_API_KEY / ALPACA_SECRET_KEY no estan configuradas.",
            color=0xE24B4A
        )
        return
    ...
    try:
        for simbolo in [SIMBOLO_LIQUIDEZ, SIMBOLO_DOLAR, SIMBOLO_ORO]:
            ...
            lineas.append(f"{simbolo}: ultimo precio {precio}")

        enviar_discord(
            "Prueba de conexion con Alpaca -- funcionando",
            "Estos son los datos reales que el bot esta leyendo ahora mismo:\n" + "\n".join(lineas),
            color=0x639922
        )
    except Exception as e:
        enviar_discord(
            "Prueba: hubo un error al leer Alpaca",
            f"Detalle: {e}",
            color=0xE24B4A
        )
...
if __name__ == "__main__":
    ...
    # Mini servidor web en un hilo aparte, solo para que hosts como Render
    # detecten que el bot sigue "vivo" (no interfiere con las alertas)
