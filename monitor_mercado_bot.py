...
INTERVALO_TENDENCIA_ORO_HORAS = 6
DIAS_MEDIA_CORTA = 5
DIAS_MEDIA_LARGA = 20
...
def revisar_tendencia_oro():
    """Calcula una tendencia tecnica simple del oro usando dos medias moviles
    (5 dias vs 20 dias) sobre el historial de xaus.com. Esto es un patron
    tecnico basico, el mismo tipo que usa cualquier plataforma de trading
    (cruce de medias moviles) -- NO es una prediccion garantizada ni consejo
    ...
    try:
        ...
        puntos = data.get("points", [])
        if len(puntos) < DIAS_MEDIA_LARGA:
            print("Tendencia oro: no hay suficiente historial todavia.")
            return

        cierres = [p["c"] for p in puntos]
        media_corta = round(statistics.mean(cierres[-DIAS_MEDIA_CORTA:]), 2)
        media_larga = round(statistics.mean(cierres[-DIAS_MEDIA_LARGA:]), 2)
        ...
        if media_corta > media_larga:
            tendencia = "AL ALZA"
            explicacion = (
                f"La media de los ultimos {DIAS_MEDIA_CORTA} dias ({media_corta}) "
                f"esta por encima de la de {DIAS_MEDIA_LARGA} dias ({media_larga}), "
                f"un patron tecnico clasico de tendencia alcista."
            )
            color = 0x5DCAA5
            ...
            explicacion = (
                f"La media de los ultimos {DIAS_MEDIA_CORTA} dias ({media_corta}) "
                f"esta por debajo de la de {DIAS_MEDIA_LARGA} dias ({media_larga}), "
                f"un patron tecnico clasico de tendencia bajista."
            )
            color = 0xE24B4A
        else:
            tendencia = "LATERAL / SIN TENDENCIA CLARA"
            explicacion = "Las dos medias estan practicamente iguales, no hay una direccion clara."
            color = 0xEF9F27

        enviar_discord(
            f"Tendencia tecnica del oro: {tendencia}",
            f"Precio actual (cierre): {precio_actual} USD/oz\n"
            f"Media {DIAS_MEDIA_CORTA} dias: {media_corta} | Media {DIAS_MEDIA_LARGA} dias: {media_larga} "
            f"(diferencia: {diferencia_pct}%)\n\n"
            f"{explicacion}\n\n"
            f"IMPORTANTE: esto es un patron tecnico simple (cruce de medias moviles), "
            f"no es una prediccion garantizada ni consejo financiero. El precio puede "
            f"cambiar de direccion en cualquier momento por noticias, datos economicos "
            f"o eventos geopoliticos.",
            color=color
        )
    except Exception as e:
        print(f"Error tendencia oro: {e}")
